...
class DE(DEBase):
    ...
    def init_eval_pop(self, budget=None, eval=True):
        ...
        # distributes the pop_size number of evaluations across num_workers
        # waits till results contain output from all pop_size evaluations
        # one-to-one correspondence of index of results and self.fitness
        with Parallel(n_jobs=self.num_workers) as parallel:
            results = parallel(
                delayed(self.f_objective)(self.population[i]) for i in range(self.pop_size)
            )
    ...
    def selection(self, trials, budget=None):
        ...
        # distributes the evaluation of trials across num_workers
        # waits till results contain output from all len(trials) evaluations
        # one-to-one correspondence of index of results and self.fitness
        with Parallel(n_jobs=self.num_workers) as parallel:
            results = parallel(
                # evaluation of the newly created individuals
                delayed(self.f_objective)(trials[i]) for i in range(len(trials))
        )
